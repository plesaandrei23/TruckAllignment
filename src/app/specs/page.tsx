...
export default function SpecsPage() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col">
      <AppHeader title={t("Tolerance profiles")} subtitle={t("Decide what passes and fails")} back="/" />
      ...
      <div className="space-y-6 px-4 py-4">
        ...
        {specs === undefined ? (
          ...
          <>
            <Group title={t("Trucks")} icon={<Truck className="size-4" />} specs={trucks} />
            <Group title={t("Trailers")} icon={<Container className="size-4" />} specs={trailers} />
          </>
        )}
      </div>
    </div>
              ...
              >
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-xs text-muted-foreground">
                    {t("Toe")} {s.toe.min}…{s.toe.max} mm/m · {t("Camber")} {s.camber.min}…{s.camber.max}°
                  </span>
                </span>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<Button className="h-11 w-full" />}>
        <Plus className="size-4" /> {t("New profile")}
      </DialogTrigger>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>{t("New tolerance profile")}</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3 pt-1">
          ...
          >
            ...
            <span className="font-medium">{t("Trailer")}</span>
          </button>
        </div>
      </DialogContent>
    </Dialog>
